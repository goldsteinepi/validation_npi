...
npi_data <- fread(file="~/Documents/NPI validation/npidata.csv", stringsAsFactors=F)
...
claims_data <- read.csv(file="~/Documents/NPI validation/Medicare_Physician_Other_Practitioners_by_Provider_and_Service_2021.csv", stringsAsFactors=F)
...
save.image("~/Documents/NPI validation/merged_npi_claims_data.RData")

#####################TAKE A RANDOM SAMPLE OF 250 FOR EACH ROLE#######################
#PHYSICIANS
#create list of unique physicians
unique_NPI<-unique(physicians$Rndrng_NPI)
set.seed(123)
random_NPI_250<-sample(random_NPI, size=250)

#create a df of random sample of 250 physicians and claims
random_physicians_250<-random_physicians_1000[random_physicians_1000$Rndrng_NPI %in% random_NPI_250, ]

#exporting
write.csv(random_physicians_250, file = "random_physicians_250.csv", row.names = FALSE,na="")

#NURSE PRACTITIONERS
#create list of unique NPs
unique_NPI_nps<-unique(nps$Rndrng_NPI)

#take a random sample of 250 providers
set.seed(123)
random_NPI_nps_250<-sample(random_NPI_nps, size=250)

#create a df of random sample of 250 physicians and claims
random_nps_250<-random_nps_1000[random_nps_1000$Rndrng_NPI %in% random_NPI_nps_250, ]

#exporting
write.csv(random_nps_250, file = "random_nps_250.csv", row.names = FALSE,na="")

#PHYSICIAN ASSISTANTS
#create list of unique PAs
unique_NPI_pas<-unique(pas$Rndrng_NPI)

#take a random sample of PAs
set.seed(123)
random_NPI_pas<-sample(unique_NPI_pas, size=1000)

#create a df of random sample of 1000 PAs and claims
random_pas_1000 <- pas[pas$Rndrng_NPI %in% random_NPI_pas, ]

#another random sample of 250 providers (within 1000)
set.seed(123)
random_NPI_pas_250<-sample(random_NPI_pas, size=250)

#create a df of random sample of 250 PAs and claims
random_pas_250<-random_pas_1000[random_pas_1000$Rndrng_NPI %in% random_NPI_pas_250, ]

#exporting
write.csv(random_pas_250, file = "random_pas_250.csv", row.names = FALSE,na="")
...
###PHYSICIANS####
random_physicians_250 <- read_csv("OneDrive - Drexel University/NPI validation/random_physicians_250.csv")
...
random_physicians_250$primary_pcp = ifelse((random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_1`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_1` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_2`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_2` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_3`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_3` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_4`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_4` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_5`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_5` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_6`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_6` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_7`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_7` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_8`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_8` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_9`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_9` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_10`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_10` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_11`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_11` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_12`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_12` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_13`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_13` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_14`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_14` %in% physician_pcp_codes) |
                                             (random_physicians_250$`Healthcare Provider Primary Taxonomy Switch_15`=="Y" & random_physicians_250$`Healthcare Provider Taxonomy Code_15` %in% physician_pcp_codes), 1, 0)

#create any taxonomy pcp indicator: 1=yes, 0=no
random_physicians_250$any_pcp = ifelse((random_physicians_250$`Healthcare Provider Taxonomy Code_1` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_2` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_3` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_4` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_5` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_6` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_7` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_8` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_9` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_10` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_11` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_12` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_13` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_14` %in% physician_pcp_codes) |
                                         (random_physicians_250$`Healthcare Provider Taxonomy Code_15` %in% physician_pcp_codes), 1, 0)
...
confusionMatrix(data = physicians$primary_pcp, reference = physicians$claim_pcp,positive="1")
#specificity: 78

#calculating 95% CI
data<-as.table(matrix(c(29, 48, 2, 171), nrow=2, byrow=TRUE))
...
confusionMatrix(data = physicians$any_pcp, reference = physicians$claim_pcp,positive="1")
#specificity: 73

#calculating 95% CI
data<-as.table(matrix(c(29, 60, 2, 159), nrow=2, byrow=TRUE))
epi.tests(data, conf.level=0.95)

write.csv(physicians, file = "physicians.csv", row.names = FALSE,na="")

###PHYSICIAN ASSISTANTS###
random_pas_250 <- read_csv("OneDrive - Drexel University/NPI validation/random_pas_250.csv")
#specify the PCP primary taxonomy codes of interest
pa_pcp_codes<-c("363A00000X","363AM0700X")

#create primary taxonomy pcp indicator: 1=yes, 0=no
random_pas_250$primary_pcp = ifelse((random_pas_250$`Healthcare Provider Primary Taxonomy Switch_1`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_1` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_2`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_2` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_3`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_3` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_4`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_4` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_5`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_5` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_6`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_6` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_7`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_7` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_8`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_8` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_9`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_9` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_10`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_10` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_11`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_11` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_12`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_12` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_13`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_13` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_14`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_14` %in% pa_pcp_codes) |
                                      (random_pas_250$`Healthcare Provider Primary Taxonomy Switch_15`=="Y" & random_pas_250$`Healthcare Provider Taxonomy Code_15` %in% pa_pcp_codes), 1, 0)

#create any taxonomy pcp indicator: 1=yes, 0=no
random_pas_250$any_pcp = ifelse((random_pas_250$`Healthcare Provider Taxonomy Code_1` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_2` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_3` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_4` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_5` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_6` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_7` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_8` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_9` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_10` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_11` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_12` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_13` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_14` %in% pa_pcp_codes) |
                                  (random_pas_250$`Healthcare Provider Taxonomy Code_15` %in% pa_pcp_codes), 1, 0)

#specify the medicare HCPCS codes of interest
hcpcs_codes <- c("G0439", "G0438", "G0468", "S5190", "G0402")

#create medicare indicator for PCP: 1=yes, 0=no
random_pas_250 <- random_pas_250 %>%
  group_by(Rndrng_NPI) %>%
  mutate(claim_pcp = as.numeric(any(hcpcs_codes %in% HCPCS_Cd))) %>%
  ungroup()
...
quantile(boot_kappas$t, c(0.025, 0.975))
#     2.5%     97.5% 
#0.006822511 0.025012590 

###simulation-based method adapted from Burstyn et al.: https://pubmed.ncbi.nlm.nih.gov/24302507/
##DEFINING VALUES
k<-10000 #size of simulation
KP.LO<-runif(k, 0.006, 0.008) #UNIFORM DISTN of lower bound on kappa
KP.HI<-runif(k, 0.015, 0.035) # UNIFORM DISTN of high bound of kappa
#PCP prev: 34.26%
PREV.CLBRT<-rbeta(k, 10.594, 19.41) #BETA distribution of exposure prevalence: https://shiny.vet.unimelb.edu.au/epi/beta.buster/
...
random_nps_250 <- read_csv("OneDrive - Drexel University/NPI validation/random_nps_250.csv")

#specify the PCP primary taxonomy codes of interest
np_pcp_codes<-c("363L00000X","363LA2200X", "363LC1500X", "363LF0000X", "363LG0600X", "363LX0001X", "363LX0106X","363LP2300X","363LW0102X")

#create primary taxonomy pcp indicator: 1=yes, 0=no
random_nps_250$primary_pcp = ifelse((random_nps_250$`Healthcare Provider Primary Taxonomy Switch_1`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_1` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_2`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_2` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_3`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_3` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_4`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_4` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_5`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_5` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_6`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_6` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_7`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_7` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_8`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_8` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_9`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_9` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_10`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_10` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_11`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_11` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_12`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_12` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_13`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_13` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_14`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_14` %in% np_pcp_codes) |
                                      (random_nps_250$`Healthcare Provider Primary Taxonomy Switch_15`=="Y" & random_nps_250$`Healthcare Provider Taxonomy Code_15` %in% np_pcp_codes), 1, 0)

#create any taxonomy pcp indicator: 1=yes, 0=no
random_nps_250$any_pcp = ifelse((random_nps_250$`Healthcare Provider Taxonomy Code_1` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_2` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_3` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_4` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_5` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_6` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_7` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_8` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_9` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_10` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_11` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_12` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_13` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_14` %in% np_pcp_codes) |
                                  (random_nps_250$`Healthcare Provider Taxonomy Code_15` %in% np_pcp_codes), 1, 0)
...
quantile(boot_kappas$t, c(0.025, 0.975))
#     2.5%     97.5% 
#0.02500686 0.06602954

##simulation-based method adapted from Burstyn et al.: https://pubmed.ncbi.nlm.nih.gov/24302507/
##DEFINING VALUES
k<-10000 #size of simulation
KP.LO<-runif(k, 0.015, 0.035) #UNIFORM DISTN of lower bound on kappa
KP.HI<-runif(k, 0.056, 0.076) # UNIFORM DISTN of high bound of kappa
#PCP prev: 55.03%
...
quantile(boot_kappas$t, c(0.025, 0.975))
#     2.5%     97.5% 
#0.01441155 0.04343330 

###simulation-based method adapted from Burstyn et al.: https://pubmed.ncbi.nlm.nih.gov/24302507/
##DEFINING VALUES
k<-10000 #size of simulation
KP.LO<-runif(k, 0.004, 0.024) #UNIFORM DISTN of lower bound on kappa
KP.HI<-runif(k, 0.033, 0.053) # UNIFORM DISTN of high bound of kappa
#PCP prev: 55.03%
PREV.CLBRT<-rbeta(k, 7.743, 6.51) #BETA distribution of exposure prevalence:https://shiny.vet.unimelb.edu.au/epi/beta.buster/
